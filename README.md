...
- Paddle OCR 13 语言扩展
- 多模态 OCR 扩展
- 多模态 OCR 保守 prompt、重试和 fallback

### 0.4.x

下一阶段不优先堆识别算法，而是优先提升真实自动化项目的稳定性、诊断能力和调用成本控制：

- 视觉证据链：原图、ROI、识别框、原始 OCR/模型返回、解析结果、最终点击点。
- OCR 结果缓存：按截图 hash、ROI、engine、prompt/version 和关键 options 缓存短生命周期结果。
- 文本点击消歧：支持第几个匹配、ROI 约束、相邻文本约束、距离模板最近等选择策略。

### 0.5.x+

- OCR fallback 策略链：按置信度、语言、ROI、成本选择 Paddle 或多模态引擎。
- 动作前置校验：点击前确认目标稳定、未遮挡、仍在期望区域内。
- 视觉状态识别：仅在真实项目需要时补 disabled/loading/selected/table cell 等状态能力。

不作为当前方向：

- 不把 Appium、Selenium、ADB、启动流程或测试编排放进 core。
- 不读取移动端 XML hierarchy 来实现视觉识别。
- 不做“大模型理解页面后自动决策”的通用 UI agent。
- 不让多模态 OCR 猜测、补全、翻译或总结 UI 文本。
